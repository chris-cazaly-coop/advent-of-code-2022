...
formattedInput.forEach((bag, index) => {
  // index = 0
  // index = 1
  // index = 2     (index + 1) % 3 == 0 => push 3 bags into baggroups

  if ((index + 1) % 3 === 0) {
    bagGroups.push([
      formattedInput[index - 2],
      formattedInput[index - 1],
      formattedInput[index],
    ]);
  }
});
...
bagGroups.forEach((bagGroup) => {
  const bag1 = bagGroup[0].split("");
  const bag2 = bagGroup[1];
  const bag3 = bagGroup[2];
  let commonLetter;
  bag1.forEach((letter) => {
    if (bag2.includes(letter) && bag3.includes(letter)) {
      commonLetter = letter;
    }
  });
